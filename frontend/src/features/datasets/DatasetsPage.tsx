      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="grid gap-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:grid-cols-[1fr_360px]">
          <button type="button" onClick={() => inputRef.current?.click()} className="min-h-44 rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/50 p-6 text-center hover:border-blue-400">
            ...
            <div className="mt-1 text-xs text-slate-500">TXT、JSONL、JSON、XML；第一阶段不支持 ZIP</div>
          ...
          <div className="space-y-3">
            ...
            <label className="block text-xs font-medium text-slate-600">文件格式<select value={format} onChange={event => setFormat(event.target.value as SourceFormat)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"><option value="auto">自动识别（推荐）</option><option value="wos_dii">WoS / Derwent TXT</option><option value="google_patents_jsonl">Google Patents JSONL</option><option value="uspto_grant_xml">USPTO Grant XML</option><option value="uspto_file_wrapper_json">USPTO File Wrapper JSON</option></select></label>
            ...
            {job && <div className={`rounded-lg p-3 text-xs ${job.status === "failed" ? "bg-rose-50 text-rose-700" : "bg-slate-50 text-slate-600"}`}><div className="flex justify-between"><span>{job.status === "completed" ? "导入完成" : job.status === "failed" ? "导入失败" : "正在解析与审计"}</span><span>{job.metrics?.progress || 0}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-blue-600 transition-all" style={{ width: `${job.metrics?.progress || 0}%` }} /></div>{job.error && <p className="mt-2">{job.error}</p>}</div>}
          </div>
        </section>
        ...
        <details className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-[11px] text-slate-400">目录必须位于后端配置的 PATENT_DATA_ROOT 内；适合大型本地数据或兼容既有工作流。</p>
        </details>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {datasets.map(dataset => {
              ...
              return <article key={dataset.id} className={`rounded-2xl border bg-white p-4 shadow-sm ${active ? "border-blue-300 ring-2 ring-blue-100" : "border-slate-200"}`}>
                ...
                <p className="mt-1 text-xs text-slate-500">{dataset.latest_version.record_count.toLocaleString()} 件 · {dataset.version_count} 个版本 · {dataset.latest_version.adapter || "unknown"}</p>
