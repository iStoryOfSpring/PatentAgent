    ...
    <div className="my-3 border border-slate-200 rounded-lg bg-white overflow-hidden shadow-sm">
      ...
      >
        <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
          ...
          <span className="bg-slate-100 px-2 py-1 rounded text-slate-600" title={`工具标识：${step.tool}`}>
            {toolLabel(step.tool)}
          </span>
          <span className="text-xs text-slate-500">{statusLabel(step.status)}</span>
          {step.status === "completed" && step.duration_ms != null && (
            <span className="text-slate-400 text-xs ml-2">耗时 {step.duration_ms} 毫秒</span>
          )}
          {step.origin && <span className="text-[10px] text-blue-600">{originLabel(step.origin)}</span>}
        ...
        <div className="p-3 border-t border-slate-100 space-y-3 text-sm">
          ...
          {step.methodology && (
            <details className="text-xs text-slate-500"><summary className="cursor-pointer font-medium">方法与质量</summary>
              ...
              {step.parameters && <pre className="mt-2 whitespace-pre-wrap">参数：{formatDisplayJson(step.parameters)}</pre>}
              {step.data_quality && <pre className="mt-2 whitespace-pre-wrap">质量：{formatDisplayJson(step.data_quality)}</pre>}
            </details>
          )}
        </div>
      )}
      ...
      {expanded && step.error && (
        <div className="p-3 text-sm text-rose-600 bg-rose-50 border-t border-rose-100">
          {localizeErrorMessage(step.error)}
