...
| `analyze_tech_roadmap` | `annual_theme_timeline` / `2.1` | `engineering_screening` | annual themes and representative records; citation paths only when resolved | publication_date>=90%; title>=80%; patent_number>=90% | — | complete invention genealogy; causal technology route |
...
| `search_patents` | `tfidf_cosine_retrieval` / `2.1` | `engineering_screening` | cosine similarity in word/phrase TF-IDF space; deterministic patent-number tie break | title>=80%; abstract>=80%; patent_number>=90% | — | vector embedding; legal novelty search; exhaustive prior-art search |
